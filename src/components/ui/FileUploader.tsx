...
const FileUploader: React.FC<FileUploaderProps> = ({ isOpen, onClose, serverId, channelId, channelName }) => {
  ...
  const handleSuccess = useCallback((_file: unknown, response: { uploadURL?: string }) => {
    ...
    if (response.uploadURL) {
      ...
      console.log('🚀 FileUploader: Posting URL to chat:', { channelName, finalUrl });
  ...
  useEffect(() => {
    ...
    uppy.addPreProcessor(async (fileIDs) => {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        const tokenHandler = (data: { serverId: string; serviceName: string; jwtToken: string }) => {
