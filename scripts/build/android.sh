...
export VITE_GIPHY_API_KEY=${VITE_GIPHY_API_KEY:-"FmaLPME7qMRkVqwzU6yfm0yxS1h9vbTV"}
...
echo "Building ObsidianIRC Android APK (ARM64 Debug)..."
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
cd "$SCRIPT_DIR/../.."

npm install
# Build with --debug to ensure it's signed with a debug key for immediate testing
